...
-- Verificar e adicionar colunas em workout_exercises
DO $$ 
BEGIN
...
CREATE TABLE IF NOT EXISTS user_exercise_weights (
    ...
    id_workouts UUID REFERENCES workouts(id),
    id_exercises UUID REFERENCES exercises(id),
    weight DECIMAL(10, 2) NOT NULL,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    UNIQUE(user_id, id_workouts, id_exercises)
);
...
CREATE INDEX IF NOT EXISTS idx_user_weights ON user_exercise_weights(user_id, id_workouts);
...
DROP POLICY IF EXISTS "Usuário pode editar próprio treino" ON workouts;
CREATE POLICY "Usuário pode editar próprio treino" ON workouts
    FOR UPDATE USING (id_profiles = auth.uid());

DROP POLICY IF EXISTS "Personal/Academia pode editar treino de sua academia" ON workouts;
